...
export function WelcomeHeader() {
  ...
  return (
    <header className="flex items-center justify-between">
      ...
      <div className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-ember to-violet text-sm font-bold text-primary-foreground">
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-ember/20 via-card to-violet/10 p-6">
      ...
      <div className="relative flex items-start justify-between">
        ...
        <span className="flex items-center gap-1 rounded-full bg-background/40 px-2.5 py-1 text-[11px] font-semibold text-foreground backdrop-blur">
...
export function StatsRow() {
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div className="rounded-3xl border border-border bg-card p-5">
        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-ember/15 text-ember">
...
const difficultyStyles: Record<string, string> = {
  Beginner: 'bg-violet/15 text-violet',
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-border bg-card p-5">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <Link
          href="/mission"
          className="group mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-ember to-violet px-5 py-3.5 text-sm font-bold text-primary-foreground glow-ember transition-transform active:scale-[0.98]"
        >
          Start mission
...
export function CoachCard() {
  return (
    <div className="glass relative overflow-hidden rounded-3xl border border-border p-5">
      <div className="flex items-start gap-3">
        <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-ember to-violet">
      ...
      <div className="mt-4 flex justify-between gap-2">
        {weeklyHeat.map((d, i) => (
          <div key={i} className="flex flex-1 flex-col items-center gap-1.5">
            <div
              className={`aspect-square w-full rounded-xl ${heatStyles[d.level]}`}
            />
  ...
  return (
    <div className="rounded-3xl border border-border bg-card p-5">
      ...
      <div className="-mx-5 mt-4 flex gap-3 overflow-x-auto px-5 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {badges.map((b) => (
          <div key={b.name} className="flex w-16 shrink-0 flex-col items-center gap-1.5">
            <div
              className={`relative flex h-14 w-14 items-center justify-center rounded-2xl border ${
                b.unlocked
                  ? 'border-ember/30 bg-gradient-to-br from-ember/20 to-violet/10 text-ember'
                  : 'border-border bg-secondary text-muted-foreground/40 grayscale'
              }`}
            >
            ...
            <span
              className={`text-center text-[10px] leading-tight ${b.unlocked ? 'text-foreground' : 'text-muted-foreground/60'}`}
            >
              {b.name}
            </span>
          </div>
      ...
      <ul className="mt-3 space-y-3">
        {recentActivity.map((a, i) => (
          <li key={i} className="flex items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-secondary text-ember">
...
export function LeaderboardCard() {
  return (
    <button className="flex w-full items-center gap-4 rounded-3xl border border-border bg-gradient-to-r from-violet/15 via-card to-card p-5 text-left transition-transform active:scale-[0.99]">
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-violet/20 text-violet">
